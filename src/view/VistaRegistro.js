...
export class VistaRegistro {
  ...
  crear() {
    const overlay = this.crearOverlay('pantallaRegistro')
    const tarjeta = this.crearTarjetaBase('tarjetaRegistro')
    overlay.addControl(tarjeta)

    tarjeta.addControl(
      this.crearTexto({
        nombre: 'tituloRegistro',
